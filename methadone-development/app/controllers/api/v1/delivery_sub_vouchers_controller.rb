class Api::V1::DeliverySubVouchersController < ApplicationController
  ...
  def create
    ...
    if sub_voucher.save
      params[:sub_voucher][:sub_medicines].each do |sub_medicine|
        new_sub_medicine = SubMedicine.create!(remaining: sub_medicine[:remaining].to_i, medicine_id: sub_medicine[:id],
          issuing_agency_id: @current_user.issuing_agency_id)

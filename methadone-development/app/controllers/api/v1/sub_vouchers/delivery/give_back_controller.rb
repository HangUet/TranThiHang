class Api::V1::SubVouchers::Delivery::GiveBackController < ApplicationController
  ...
  def create
    ...
    last_sub_voucher = SubVoucher.where(issuing_agency_id: current_user.issuing_agency_id,
                                     typee: SubVoucher.typees[:export_to_give_back])
                              .order(code: :desc).first

    if last_sub_voucher.present?
      fk_voucher_code = "X_TL_#{current_user.issuing_agency.code}_#{(last_sub_voucher.code.split('_').last.to_i + 1).to_s.rjust(6, '0')}"
    else
      fk_voucher_code = "X_TL_#{current_user.issuing_agency.code}_000001"
    end
  ...
  def update
    ...
    sub_voucher = SubVoucher.where(id: sub_voucher_params[:id], issuing_agency_id: @current_user.issuing_agency_id).first
    ...
    if sub_voucher.present?
      ...
      sub_medicines.each do |sub_medicinex|
        if sub_medicinex[:sub_medicine_id] == nil

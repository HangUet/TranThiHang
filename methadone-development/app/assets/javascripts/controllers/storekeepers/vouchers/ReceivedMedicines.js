app.controller("Storekeeper_Voucher_ReceivedMedicine",
  ...
  $scope.showEditVoucherModal = function () {
    ...
    var modalInstance = $uibModal.open({
      templateUrl: "/templates/vouchers/received/edit.html",
      backdrop: 'static',
      keyboard: false,
      resolve: {
        ...
        list_medicines: ['API', function(API) {
          return API.getListMedicines().then(function(response) {
      ...
      controller: ['$scope', '$rootScope', 'voucher', 'list_medicines', 'manufacturers', 'providers',
        '$uibModalInstance', 'toastr', '$state', '$stateParams', 'API', 'ReceivedVoucher',
        ...
        $scope.sum_money_cacl = function () {
          ...
          for (var i in $scope.new_voucher.medicines) {
            $scope.new_voucher.total_money += ($scope.new_voucher.medicines[i].price || 0) * ($scope.new_voucher.medicines[i].number_order || 0);
          }
        }
        $scope.sum_money_cacl();
        ...
        $scope.editVoucher = function () {
          NProgress.start();
          $scope.sum_money_cacl();
          ReceivedVoucher.update($stateParams.id, $scope.new_voucher).success(function (response) {
            NProgress.done();
            if(response.code == 1) {
              $state.reload();
  ...
  $scope.showPrintVoucherModal = function() {
    NProgress.start();
    var modalInstance = $uibModal.open({
      templateUrl: "/templates/vouchers/received/medicines/print.html",
      size: 'lg',
      backdrop: 'static',
      keyboard: false,
      resolve: {

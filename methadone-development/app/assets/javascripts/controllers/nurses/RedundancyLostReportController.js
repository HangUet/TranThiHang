app.controller("RedundancyLostReportController", ['$scope', '$rootScope',
 '$state', 'toastr', 'API', '$uibModal', '$filter', '$ngBootbox',
  ...
  $scope.showPrintModal = function(councils) {
    ...
    var modalInstance = $uibModal.open({
      templateUrl: "/templates/nurses/lost_redundancy.html",
      size: 'lg',
      controller: ['$scope', '$uibModalInstance', 'toastr', '$state', 'API',
        ...
        API.getLossReduncy("month", time, "loss_redundancy").success(function (response) {
          NProgress.done();
          if(response.code == 1) {
            $scope.redundancy = response.redundancy;
            $scope.loss = response.loss;
            $scope.list_medicine = response.list_medicine;
            $scope.total_allocation = response.total_allocation;
          } else {
        ...
        var date = TIME_NOW;
        $scope.currentMonth = choose_time[0];
        $scope.currentYear = choose_time[1];

app.controller("Doctor_NewPatientController", ['$scope', '$state', 'toastr', '$rootScope',
  'ethnicities', 'provinces', 'AvatarUploader', 'API', '$uibModal',
  function ($scope, $state, toastr, $rootScope, ethnicities, provinces, AvatarUploader, API, $uibModal) {
  var province_id = "";
  var district_id = "";

  if($rootScope.currentUser.agency.province_id) {
    province_id = $rootScope.currentUser.agency.province_id;
    $scope.loadDistrict(province_id);
    $scope.loadDistrictResident(province_id);
  }

  if($rootScope.currentUser.agency.district_id) {
    district_id = $rootScope.currentUser.agency.district_id;
    $scope.loadWard(district_id);
    $scope.loadWardResident(district_id);
  }

  $scope.patient = {
    province_id: province_id,
    district_id: district_id,
    resident_province_id: province_id,
    resident_district_id: district_id,
    ethnicity_id: 1,
    contacts: [{
      contact_type: ""
    }]
  ...
  $scope.resident_provinces = provinces;
  ...
  $scope.createPatient = function() {
    NProgress.start();
    API.createPatient($scope.patient).success(function (response) {
      NProgress.done();
      if(response.code == 1) {
            ...
            controller: ['$scope', '$uibModalInstance', 'toastr', '$state', 'API', '$ngBootbox', '$filter',
              ...
              $scope.createPatient = function() {
                ...
                $ngBootbox.confirm($filter("translator")("confirm_add_patient", "main") + patient_tmp.name + '?').then(function() {
                  ...
                  API.createPatient($scope.patient_tmp).success(function (response) {
                    NProgress.done();
                    if(response.code == 1) {
                      toastr.success(response.message);
                      $state.go('main.patients.action.detail.executive_info', {id: response.id}, {reload: true});

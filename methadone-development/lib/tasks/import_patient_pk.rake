namespace :import_patient_pk do
...
#["CSDTMMTTrungtamchuabenhgiaoduclaodongxahoiso5.xlsx", "CSĐT MMT Trung tâm chữa bệnh, giáo dục, lao động xã hội số 5"],
    ...
    def get_patient_contact_type patient_contact_type
      patient_contact_type = patient_contact_type rescue nil
    ...
    require 'roo'
    agency_list = [["PhongKham.xlsx", "CSĐT MMT PK chuyên khoa và điều trị nghiện chất TTYTDP Hà Nội"]]
            ...
            if index > 5
              ...
              if name.present?
                ...
                if patient[19].present?
                  ...
                  patient_contact_type = nil
                  temp = patient[19].squish rescue nil
                  if (temp.include? "Mẹ")
                    patient_contact_type = 1
                    patient_contact_name = temp.split("Mẹ")[1].squish rescue nil
                  elsif temp.include? "Bố"
                    patient_contact_type = 0
                    patient_contact_name = temp.split("Bố")[1].squish rescue nil
                  elsif temp.include? "Anh trai"
                    patient_contact_type = 1
                    patient_contact_name = temp.split("Anh trai")[1].squish rescue nil
                  elsif temp.include? "Em trai"
                    patient_contact_type = 1
                    patient_contact_name = temp.split("Em trai")[1].squish rescue nil
                  elsif temp.include? "Em gái"
                    patient_contact_type = 1
                    patient_contact_name = temp.split("Em gái")[1].squish rescue nil
                  elsif temp.include? "Chị gái"
                    patient_contact_type = 1
                    patient_contact_name = temp.split("Chị gái")[1].squish rescue nil
                  elsif temp.include? "Vợ"
                    patient_contact_type = 2
                    patient_contact_name = temp.split("Vợ")[1].squish rescue nil
                  elsif temp.include? "Con gái"
                    patient_contact_type = 4
                    patient_contact_name = temp.split("Con gái")[1].squish rescue nil
                  elsif temp.include? "Con trai"
                    patient_contact_type = 4
                    patient_contact_name = temp.split("Con trai")[1].squish rescue nil
                  else

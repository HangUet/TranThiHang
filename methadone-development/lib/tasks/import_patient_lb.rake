namespace :import_patient_lb do
  ...
  task init: :environment do
    ...
    require 'roo'
    agency_list = [["CSDTMMTQuanLongBien.xls", "CSĐT MMT Quận Long Biên"]]
          ...
          patients.each_with_index do |patient, index|
            if !patient[0].nil? && !patient[1].nil? && !patient[2].nil?
              if patient[0]!= "STT" && patient[0] != "Năm:"
                patient_info = []
                code = patient[1]
                identity_card_number = patient[9].squish rescue nil
                  ...
                  if patient[20].present?
                   patient_contact_name = patient[20].squish
                    ...
                    if patient[22].present?
                      patient_contact_phone = get_patient_contact_phone patient[22]
                    end
                    patient_contact_type = nil
                      if patient[19].present?
                        patient_contact_type = get_patient_contact_type patient[19]
                      else
                        patient_contact_type = 5
                      end
                  end
